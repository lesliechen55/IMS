window.localStorage.setItem("changeRefresh", window.localStorage.getItem("changeRefresh") == null ? "true" : window.localStorage.getItem("changeRefresh"));
var tabFilter, menu = [], liIndex, curNav, delMenu,
    changeRefreshStr = window.localStorage.getItem("changeRefresh");
layui.define(["element", "jquery"], function (exports) {
            ...
            if (data[i].children != undefined && data[i].children.length > 0) {
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    if (data[i].children[j].target == "_blank") {
                        ulHtml += '<dd><a data-url="' + data[i].children[j].href.toLowerCase() + '" target="' + data[i].children[j].target + '">';
                    } else {
                        ulHtml += '<dd><a data-url="' + data[i].children[j].href.toLowerCase() + '">';
                    }
                ...
                if (data[i].target == "_blank") {
                    ulHtml += '<a data-url="' + data[i].href.toLowerCase() + '" target="' + data[i].target + '">';
                } else {
                    ulHtml += '<a data-url="' + data[i].href.toLowerCase() + '">';
                }
                if (data[i].icon != undefined && data[i].icon != '') {
                    if (data[i].icon.indexOf("icon-") != -1) {
                        ulHtml += '<i class="seraph ' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
    ...
    Tab.prototype.render = function () {
        //显示左侧菜单
        var _this = this;
        $(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="/home/main"><i class="layui-icon" data-icon=""></i><cite>我的首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height() - 210);
    ...
    //是否点击窗口切换刷新页面
    Tab.prototype.changeRegresh = function (index, href, changeRefreshStr, title) {
        if (changeRefreshStr == "true") {
            var oldhref = $(".clildFrame .layui-tab-item").eq(index).find("iframe")[0].contentWindow.location.href;
            if (oldhref != href && href) {
                $(".clildFrame .layui-tab-item").eq(index).find("iframe")[0].contentWindow.location.href = href;
            }
            else {
                $(".clildFrame .layui-tab-item").eq(index).find("iframe")[0].contentWindow.location.reload();
            }
            var oldtitle = $("#top_tabs li").eq(index).find("cite").text();
            if (title && oldtitle != title) {
                oldtitle = $("#top_tabs li").eq(index).find("cite").text(title);
            }
        }
    }
    ...
    Tab.prototype.getLayId = function (name) {
        $(".layui-tab-title.top_tab li").each(function () {
            if ($(this).find("cite").attr('name') == name) {
    ...
    //通过name获取lay-index
    Tab.prototype.getLayIndex = function (name) {
        var layIndex = -1;
        $(".layui-tab-title.top_tab li").each(function (index) {
            if ($(this).find("cite").attr('name') == name) {
                layIndex = index;
                return layIndex;
            }
    ...
    Tab.prototype.tabAdd = function (_this) {
        ...
        if (_this.attr("target") === "_blank") {
            ...
            var arr = _this.attr("data-url").split('?')[0].split('/');
            var url = encodeURI(_this.attr("data-url"));
            var iframeName = arr.join('');
            //已打开的窗口中不存在
            if (that.hasTab(iframeName) === -1 && _this.siblings("dl.layui-nav-child").length == 0) {
                ...
                title += '<cite name="' + iframeName + '">' + _this.find("cite").text() + '</cite>';
                title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="' + tabIdIndex + '">&#x1006;</i>';
                element.tabAdd(tabFilter, {
                    title: title,
                    content: '<iframe name="' + iframeName + '" src="' + url + '" data-id="' + tabIdIndex + '"></iframe>',
                    id: new Date().getTime()
                })
                //当前窗口内容
                var curmenu = {
                    ...
                    "topMenuName": _this.attr("topMenuName"),
                    "leftMenuName": _this.attr("leftMenuName")
                }
                menu.push(curmenu);
                //that.changeRegresh(tabIdIndex, curmenu.href, "true", curmenu.title);
                ...
                element.tabChange(tabFilter, that.getLayId(iframeName));
                that.tabMove(); //顶部窗口是否可滚动
            } else {
                //当前窗口内容
                var curindex = that.getLayIndex(iframeName);
                var curmenu = {
                    "icon": _this.find("i.seraph").attr("data-icon") != undefined ? _this.find("i.seraph").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),
                    "title": _this.find("cite").text(),
                    "name": iframeName,
                    "href": url,
                    "layId": menu[curindex].layId,
                    "topMenuName": _this.attr("topMenuName"),
                    "leftMenuName": _this.attr("leftMenuName")
                }
                that.changeRegresh(curindex, curmenu.href, "true", _this.find("cite").text());
                menu[curindex] = curmenu;
                window.sessionStorage.setItem("curmenu", JSON.stringify(curmenu));  //当前的窗口
                window.sessionStorage.setItem("menu", JSON.stringify(menu)); //打开的窗口
                element.tabChange(tabFilter, that.getLayId(iframeName));
                that.tabMove(); //顶部窗口是否可滚动
            }
            window.location.href = window.location.href.split('#')[0] + '#/' + _this.attr("topMenuName") + url;
        }
    }
    ...
    $("body").on("click", ".top_tab li", function () {
        ...
        if (window.sessionStorage.getItem("curmenu") == "undefined") {
            ...
            if (curNav != JSON.stringify(delMenu)) {
                ...
                curmenu = JSON.parse(curNav);
            } else {
                window.sessionStorage.setItem("curmenu", JSON.stringify(menu[liIndex]));
            }
        }
        ...
        if (curmenu) {
            ...
            bodyTab.changeRegresh($(this).index(), curmenu.href, changeRefreshStr);
            topMenuName = curmenu.topMenuName;
    ...
    $("body").on("click", ".top_tab li i.layui-tab-close", function () {
        ...
        if (menu != null) {
            ...
            if (curmenu.name != delMenu.name) {  //如果删除的不是当前选中的tab
    ...
    $(".closePageOther").on("click", function () {
        if ($("#top_tabs li").length > 2 && $("#top_tabs li.layui-this cite").text() != "我的首页") {
            var menu = JSON.parse(window.sessionStorage.getItem("menu"));
                ...
                if ($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")) {
                    ...
                    for (var i = 1; i < menu.length; i++) {
                        if ($("#top_tabs li.layui-this cite").text() == menu[i].title) {
                            menu.splice(1, menu.length, menu[i]);
                            window.sessionStorage.setItem("menu", JSON.stringify(menu));
                        }
                    }
                }
            })
        } else if ($("#top_tabs li.layui-this cite").text() == "我的首页" && $("#top_tabs li").length > 1) {
            $("#top_tabs li").each(function () {
                if ($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")) {
                    element.tabDelete("bodyTab", $(this).attr("lay-id")).init();
                    menu.splice(1, menu.length);
                    window.sessionStorage.setItem("menu", JSON.stringify(menu));
                }
            })
        } else {
            ...
            $("#top_tabs li").each(function () {
                if ($(this).attr("lay-id") != '') {
                    ...
                    if (menu.length > 1) {
                        menu.splice(1, menu.length);
                        window.sessionStorage.setItem("menu", JSON.stringify(menu));
                        window.sessionStorage.setItem("curmenu", JSON.stringify(menu[0]));
                    }
                }
            })
        } else {
