...
using YQTrack.Core.Backend.Admin.Web.Areas.Pay.Controllers;
...
{
    public class PlatformShopController : BaseRealTimeController
    {
        IPlatformShopService _platformService;
        IMapper _mapper;

        public PlatformShopController(IPlatformShopService platformService,IMapper mapper)
        {
        ...
        public async Task<IActionResult> GetData(PlatformShopDataRequest request)
        {
           ...
           var (outputs, total) = await _platformService.GetDataAsync(input);
