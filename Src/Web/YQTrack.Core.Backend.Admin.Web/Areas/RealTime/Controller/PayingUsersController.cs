...
{
    public class PayingUsersController : BaseRealTimeController
    {
        IPlatformShopService _platformService;
        IMapper _mapper;

        public PayingUsersController(IPlatformShopService platformService, IMapper mapper)
        {
        ...
        public async Task<IActionResult> GetData(PlatformShopDataRequest request)
        {
            ...
            var (outputs, total) = await _platformService.GetPayingUsersDataAsync(input);
