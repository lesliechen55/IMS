...
{
    public class SearchNumController : BaseRealTimeController
    {
        IPlatformShopService _platformService;
        IMapper _mapper;

        public SearchNumController(IPlatformShopService platformService, IMapper mapper)
        {
        ...
        public async Task<IActionResult> GetData(PlatformShopDataRequest request)
        {
            ...
            var (outputs, total) = await _platformService.GetSearchNumDataAsync(input);
