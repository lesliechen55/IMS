...
{
    public class ChannelPageDataRequestValidator : AbstractValidator<ChannelPageDataRequest>
    {
        public ChannelPageDataRequestValidator()
        {
            RuleFor(x => x.Id).Must(x => !x.HasValue || x.Value > 0).WithMessage("渠道ID不能小于等于0");

            RuleFor(x => x.PublishStartTime).Custom((x, y) =>
            {
                if (y.InstanceToValidate is ChannelPageDataRequest request)
                {
