...
{
    public class TrackCountRequestValidator : AbstractValidator<TrackCountRequest>
    {
        public TrackCountRequestValidator()
        {
            RuleFor(x => x.StartTime).Custom((x, y) =>
            {
                if (y.InstanceToValidate is TrackCountRequest request)
                {
